 ...
 *
 * *** MODIFIED BY CLOUDEY IT LTD ***
 */

namespace Cloudey.Different.DiffMatchPatch;
...
public class diff_match_patch
{
     ...
     * @return List of Diff objects.
     */
    private List<Diff> diff_lineMode (
        string text1,
        string text2,
        DateTime deadline
    )
                ...
                case Operation.EQUAL:
                    ...
                    if (count_delete + count_insert > 1)
                    {
                        if (count_delete != 0 && count_insert != 0)
                        {
                            ...
                            if (commonlength != 0)
                            {
                                diffs[pointer].text = text_insert.Substring(text_insert.Length - commonlength) +
                                                      diffs[pointer].text;
    ...
    protected void patch_addContext (Patch patch, string text)
    {
        ...
        // Add the prefix.
        string prefix = text.JavaSubstring(
            Math.Max(0, patch.start2 - padding),
            patch.start2
        );
        if (prefix.Length != 0)
        ...
        // Add the suffix.
        string suffix = text.JavaSubstring(
        ...
        foreach (Patch aPatch in patches)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (text1.Length > this.Match_MaxBits &&
                        this.diff_levenshtein(diffs) / (float)text1.Length > this.Patch_DeleteThreshold)
                    {
